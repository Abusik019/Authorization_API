...
const authServer = http.createServer((req, res) => {
    ...
    if(method === "GET" && pathname === "/api/users"){
        ...
        try {
            if (reg === 'true') {
                res.writeHead(200);
                res.end(JSON.stringify(data.filter(user => user.isLogin === true)));
            } else {
                switch (filter) {
                    case 'name':
                        res.writeHead(200);
                        res.end(JSON.stringify(data.sort((a, b) => a.name.localeCompare(b.name))));
                        break;
                    case 'reg':
                        res.writeHead(200);
                        res.end(JSON.stringify(data.sort((a, b) => b.isLogin - a.isLogin)));
                        break;
                    case 'email':
                        res.writeHead(200);
                        res.end(JSON.stringify(data.sort((a, b) => a.email.localeCompare(b.email))));
                        break;
                    default:
                        res.writeHead(200);
